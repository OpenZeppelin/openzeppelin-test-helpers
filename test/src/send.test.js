...
contract('send', function ([sender, receiver]) {
  ...
  describe('transaction', function () {
    ...
    it('calls a function from its signature ', async function () {
      const receipt = await send.transaction(this.acknowledger, sender, 'foo', 'uint256', [3]);
    ...
    it('calls overloaded functions with less arguments', async function () {
      const receipt = await send.transaction(this.acknowledger, sender, 'bar', 'uint256', [3]);
    ...
    it('calls overloaded functions with more arguments', async function () {
      const receipt = await send.transaction(this.acknowledger, sender, 'bar', 'uint256,uint256', [3, 5]);
    ...
    it('throws if the number of arguments does not match', async function () {
      await shouldFail(send.transaction(this.acknowledger, sender, 'foo', 'uint256, uint256', [3, 5]));
    });

    it('throws if the method does not exist', async function () {
      await shouldFail(send.transaction(this.acknowledger, sender, 'baz', 'uint256', [3]));
    });

    it('throws if there is a mismatch in the number of types and values', async function () {
      await shouldFail(send.transaction(this.acknowledger, sender, 'foo', 'uint256', [3, 3]));
    });
  });
});
