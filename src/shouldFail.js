...
async function withMessage (promise, message) {
  ...
  if (1 in matches && semver.satisfies(matches[1], '>=2.2.0')) {
    ...
    console.log(`${c.white.bgBlack('openzeppelin-test-helpers')} ${c.black.bgYellow('WARN')} shouldFail.reverting.\
withMessage: current version of Ganache (${matches[1]}) doesn't return revert reason.`);
